...
@section('content')
    ...
    <div class="breadcrumb-area bg-img bg-overlay jarallax"
        style="background-image: url('{{ asset('img/tool2.png') }}'); background-color: white;">
                                    ...
                                    @auth
                                        @if (auth()->user()->user_type === 'user')
                                            ...
                                            <form method="POST" action="{{ route('favorites.toggle', $product->id) }}"
                                                class="favorite-form position-absolute" style="top: 15px; right: 15px;">
                                                @csrf
                                                <button type="submit"
                                                    class="favorite-btn bg-white rounded-circle p-2 shadow-sm"
                                                    style="border: none;">
                                                    <i class="fa {{ $product->is_favorited ? 'fa-heart text-danger' : 'fa-heart-o text-muted' }}"
                                    ...
                                    <div class="room-content">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <h4 class="m-0 text-primary">
                                                {{ number_format($product->price, 2) }} <span style="font-size: 0.8rem;">/
                                                    Day</span>
                                            </h4>
                                            <span
                                        ...
                                        <div class="room-feature d-flex flex-wrap gap-3" style="min-height: 70px;">
                                            ...
                                            @if (in_array($product->user_id, $activeSubscriptionUserIds))
                                                <p class="mb-0 w-100 text-success">
                                                    <i class="fa fa-comments"></i> <strong>Chat Available</strong>
                                                </p>
                                            @endif
