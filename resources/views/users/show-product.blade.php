                    ...
                    <div class="room-review-area">
                        ...
                        <div class="mb-5">
                            <ul class="nav nav-tabs" id="reviewTab" role="tablist">
                                <li class="nav-item"><a class="nav-link active" id="reviews-tab" data-toggle="tab" href="#reviews-content">Reviews ({{ $reviews->count() }})</a></li>
                                <li class="nav-item"><a class="nav-link" id="add-review-tab" data-toggle="tab" href="#add-review">Write Review</a></li>
                            </ul>
                                    ...
                                    @forelse ($reviews as $review)
                                        <div class="single-room-review-area d-flex mb-4">
                                            <div class="reviwer-thumbnail mr-4">
                                                <img src="{{ $review->user->profile_picture ? asset('storage/' . $review->user->profile_picture) : asset('img/default-user.png') }}" alt="User Image" width="120" height="120" class="shadow-sm rounded" style="object-fit: cover;" />
                                            </div>
        ...
        <div class="bg-white p-4 shadow-sm rounded">
            ...
            <form id="rentalForm" action="{{ route('user.rentals.store') }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" id="start_date" name="start_date" class="form-control" placeholder="e.g. 2025-04-24" data-product-id="{{ $product->id }}">
                </div>

                <div class="form-group mb-3">
                    <label for="end_date">End Date</label>
                    <input type="text" id="end_date" name="end_date" class="form-control" placeholder="e.g. 2025-04-27">
                </div>
                ...
                <div class="form-group mt-3">
                    ...
                    <textarea name="note" id="note" class="form-control" rows="3" placeholder="Any special instructions or notes?"></textarea>
                </div>
                ...
                <div class="form-group form-check mt-3">
                    ...
                    <!-- هنا رح يطلع الخطأ تحت التشيك -->
                </div>

                <!-- Reserve Button -->
                <button id="reserveBtn" type="button" class="btn btn-primary w-100 mt-4">Reserve Now</button>
            </form>

            <!-- Read Terms Link -->
            <a href="javascript:void(0);" id="readTermsLink" class="text-primary d-inline-block mt-2" style="text-decoration: underline;">
                Read Terms
            </a>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        Swal.fire({
            ...
            text: @json($errors->first()) // أو استخدم implode لتعرض الكل
        });
    </script>
    @endif
    ...
    <!-- تعريف productId (إذا بتستخدمه في سكربتات لاحقة) -->
